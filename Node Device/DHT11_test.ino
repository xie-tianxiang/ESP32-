...
uint8_t broadcastAddress1[] = {0x60,0x55,0xF9,0x7B,0x8B,0x40};//ESP开发板接收板的MAC地址60:55:F9:7B:9B:58
...
void loop() {
  myData.id = 3;//开发板的ID每个都不一样
  Humidity = dht.readHumidity();
  Temperature = dht.readTemperature();
  myData.x = Temperature;//温度变量传入
  myData.y = Humidity;//湿度变量传入
